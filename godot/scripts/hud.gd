...
func set_hero(h: HeroState, floor_no: int, remaining: int) -> void:
	...
	info.text = "%s  Lv%d   HP %d/%d   %d階  残り%d体" % [
		GameData.klass(h.class_id).get("name", h.class_id), h.level,
		int(h.hp), int(h.stats["max_hp"]), floor_no, remaining]
